...
const blinkSightsToken = process.env.NEXT_PUBLIC_BLINKSIGHT

const client = new BlinksightsClient(`${blinkSightsToken}`);

const headers = createActionHeaders();

function validatedQueryParams(requestUrl: URL) {
  let toPubkey: PublicKey = new PublicKey(
    "DgayKz99e9vU3odG2LNQBj1jMMmi32cS1x9Gt3ZNYRmv"
  );
...
export const GET = async (req: Request) => {
  try {
    ...
    const baseHref = new URL(
      `/api/actions/donate-pool?to=${toPubkey.toBase58()}`,
      requestUrl.origin
    ).toString();
...
export const POST = async (req: Request) => {
  try {
    ...
    client.trackActionV2(toPubkey.toString(), req.url);
